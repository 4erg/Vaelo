...
const PLATFORM_IMAGES: Record<string, string> = {
  android: '/platform-android-mobile.png',
  androidtv: '/platform-android-tv.png',
  ios: '/platform-ios.png',
  samsung: '/platform-smart-tv-premium.png',
  lg: '/platform-smart-tv-premium.png',
  vidaa: '/platform-smart-tv-premium.png',
  titan: '/platform-smart-tv-premium.png',
  windows: '/platform-windows.png',
  web: '/platform-web.png',
}
    ...
    <div className="hero-visual-shell">
      <div className="hero-visual-panel hero-visual-panel-linear">
        ...
        <div className="hero-device-image-wrap">
          <img
            src="/vaelo-ai-device-showcase.png"
            alt="Mockup realista de VAELO en TV, tablet, laptop y movil"
...
function PlatformCard({ p }: { p: typeof PLATFORMS[0] }) {
  ...
  const image = PLATFORM_IMAGES[p.id] ?? '/vaelo-ai-device-showcase.png'
  ...
  return (
    <div className="platform-card p-6 flex flex-col h-full">
      ...
      <div className="platform-visual-stage platform-photo-stage mb-5" style={{ '--platform-accent': p.color } as React.CSSProperties}>
        <img
          src={image}
          alt={`Vista realista para ${p.name}`}
          className={`platform-photo platform-photo-${p.id}`}
        />
        <div className="platform-photo-shade" />
        <div className="platform-photo-label">
          <strong>{p.name}</strong>
          <span>{p.features[0]}</span>
        </div>
        <div className="platform-photo-badge">{tag}</div>
      </div>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-[0.88fr_1.12fr] gap-12 items-center">
          ...
          <div className="brand-comparison-premium">
            <div className="brand-scene-card brand-scene-generic">
              ...
              <div className="brand-featured-shell brand-generic-photo-shell">
                <img src="/brand-generic-player-vertical.png" alt="Interfaz IPTV generica sin marca personalizada en movil" className="brand-featured-image brand-generic-image" />
              </div>
            </div>
...
function EnhancedCustomization() {
  ...
  const [previewOrientation, setPreviewOrientation] = useState<'vertical' | 'horizontal'>('vertical')
  const [logoPreview, setLogoPreview] = useState('')
  const colors = ['#2B7FFF', '#0EA5A4', '#2563EB', '#0F766E', '#1D4ED8', '#F97316']
  const secondaryColors = ['#0D1B3E', '#111827', '#1E1B4B', '#172554', '#312E81', '#064E3B']
  const items = [
  ...
  const channelTiles = ['EN VIVO', 'PELICULAS', 'SERIES', 'DEPORTES', 'KIDS', 'FAVORITOS']
  ...
  return (
    <section id="customization" className="py-24 px-6 services-shell">
      <div className="max-w-7xl mx-auto">
        <div className="customization-layout grid lg:grid-cols-[0.58fr_1.42fr] gap-10 items-start">
          <div>
            ...
            <div className="detail-panel">
              <div className="space-y-3 mb-6">
                ...
                <label className="admin-field">
                  <span>Logotipo de referencia</span>
                  <input type="file" accept="image/*" onChange={handleLogoChange} />
                </label>
              </div>
              <div className="mb-5">
                <span className="block text-sm font-display font-700 mb-3" style={{ color: '#60708A' }}>Dispositivo</span>
                <div className="custom-segmented">
                  <button type="button" className={previewDevice === 'tv' ? 'active' : ''} onClick={() => setPreviewDevice('tv')}>Smart TV</button>
                  <button type="button" className={previewDevice === 'phone' ? 'active' : ''} onClick={() => setPreviewDevice('phone')}>Telefono</button>
                </div>
              </div>
              <div className="mb-5">
                <span className="block text-sm font-display font-700 mb-3" style={{ color: '#60708A' }}>Orientacion</span>
                <div className="custom-segmented">
                  <button type="button" className={previewOrientation === 'vertical' ? 'active' : ''} onClick={() => setPreviewOrientation('vertical')}>Vertical</button>
                  <button type="button" className={previewOrientation === 'horizontal' ? 'active' : ''} onClick={() => setPreviewOrientation('horizontal')}>Horizontal</button>
                </div>
              </div>
              <div className="mb-5">
              ...
              <div className="mb-6">
                <span className="block text-sm font-display font-700 mb-3" style={{ color: '#60708A' }}>Color de fondo</span>
                <div className="flex flex-wrap gap-3">
                  {secondaryColors.map(color => (
                    <button
                      ...
                      aria-label={color}
                      className="color-dot"
                      style={{ background: color, outline: secondaryColor === color ? '3px solid rgba(13,27,62,0.18)' : 'none' }}
          ...
          <div className="custom-simulator-wrap">
            <div className="custom-before-card">
              <span>GENERICA</span>
              <strong>IPTV Player</strong>
              <div className="custom-before-grid">
                {[1, 2, 3, 4].map(i => <i key={i} />)}
              </div>
            </div>

            <div className={`custom-live-preview ${previewDevice === 'phone' ? 'phone-mode' : 'tv-mode'} ${previewOrientation === 'horizontal' ? 'landscape-mode' : 'portrait-mode'}`}>
              {previewDevice === 'phone' ? (
                <div className="custom-phone-app" style={{ background: `linear-gradient(180deg, ${secondaryColor}, #06080D)` }}>
                  <div className="custom-phone-toolbar">
                    <div className="custom-phone-logo" style={{ background: selectedColor }}>
                  ...
                  <div className="custom-phone-feature">
                    <div className="custom-phone-pills">
                      <span style={{ background: selectedColor }}>DESTACADO APK IPTV</span>
                      <i>CANAL EN VIVO</i>
                    </div>
                    <div className="custom-phone-channel">
                      <div>
                ...
                <div className="custom-preview-device" style={{ background: `linear-gradient(145deg, ${secondaryColor}, #061226)` }}>
                  <div className="custom-preview-topbar" style={{ background: `linear-gradient(135deg, ${selectedColor}, ${secondaryColor})` }}>
                    <div className="custom-preview-brand">
                      {logoPreview ? <img src={logoPreview} alt="" /> : <span>{brandName.slice(0, 1).toUpperCase()}</span>}
                      <div>
                        <strong>{brandName || 'Tu Marca'}</strong>
                        <small>{tagline || 'Canales · Peliculas · Series'}</small>
                      </div>
                    </div>
                    <i>LIVE</i>
                  </div>
                  <div className="custom-preview-hero">
                    <div>
                      <strong>Estrenos destacados</strong>
                      <span>Banner con colores, logo y nombre comercial.</span>
                    </div>
                    <button type="button" style={{ background: selectedColor }}>Ver ahora</button>
                  </div>
                  <div className="custom-preview-tabs">
                    {channelTiles.map((item, index) => (
                      <span key={item} style={{ borderColor: index === 0 ? selectedColor : undefined }}>
                        {item}
                      </span>
                    ))}
                  </div>
                  <div className="custom-preview-grid">
                    {[1, 2, 3, 4, 5, 6].map(i => <i key={i} style={{ background: `linear-gradient(135deg, ${selectedColor}35, rgba(255,255,255,0.08))` }} />)}
                  </div>
                </div>
              )}
            </div>
